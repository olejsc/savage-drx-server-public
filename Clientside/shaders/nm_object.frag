...
void main()
{
	...
	float h = texture2D(normalMap, gl_TexCoord[0].st).a;																	// read height value
	...
	baseColor = texture2D(texture0, newTexcoord) * gl_Color;	// read base texture and modulate by glColor
		...
		color.rgb += specular.rgb * gl_FrontMaterial.specular.rgb;								// apply specular
		color.rgb *= ps_pb_cs_hs.z;																								// apply GL_MODULATE
							
		color.rgb = mix(gl_Fog.color.rgb, color.rgb, fog);												// apply linear fog
