...
float waveInfluence()
{
	float H = 0.0;
	float freq;
	vec4 amp=amplitude, wl=wavelength, spd=speed*2.0;
...
{
	vec2 c1 = vec2(0.0, 1.0)*texScale;
	vec2 c2 = vec2(1.0, 0.0)*texScale;
...
void main()
{
	...
	coverage.a=smoothstep(0.8, 1.0, coverage.a);

	float n = dot(upnormal, normal);
	...
	vec4 vPos = vec4(inVertex.xyz+camUp.xyz*10.0*inKernelOffset.y*coverage.a+windInf*coverage.a, inVertex.w);

	vPos.z += coverage.b;

	//mat4 scmat = mat4(vec4(coverage.a, 0.0,0.0,0.0), vec4(0.0,coverage.a,0.0,0.0), vec4(0.0,0.0,coverage.a,0.0), vec4(0.0,0.0,0.0,1.0));
	...
	NdotL = clamp(dot(upnormal, lightDir)+0.5, 0.0, 1.0);

	windInf = vec3(1.0,1.0,1.0)*clamp(H,0.4,1.0);

	diffuse = (vec3(1.0) * gl_LightSource[0].diffuse.rgb * NdotL)*windInf;
